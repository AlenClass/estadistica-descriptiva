...
import datosInmobiliarios from "./datos_inmobiliarios.json";
...
// 2. Porcentaje de personas que pesan menos de 65 kg
const menos65 = pesos.filter((p) => p < 65).length;
const porcentajeMenos65 = ((menos65 / totalPesos) * 100).toFixed(2);

// 3. Personas que pesan 70 kg o más pero menos de 85 kg
...
const labelsPesos = intervalos.map(
  ([ini, fin]) => `${ini}-${fin}`
);
const puntosMedios = intervalos.map(([ini, fin]) => (ini + fin) / 2);
...
// 1. Extraer precios
const precios = datosInmobiliarios
  .map((row) => row.Precio)
...
// 5. Tabla de contingencia (TipoPropiedad vs Ubicacion)
const tipos = [...new Set(datosInmobiliarios.map((d) => d.TipoPropiedad))];
const ubicaciones = [...new Set(datosInmobiliarios.map((d) => d.Ubicacion))];

let tablaContHTML = `
  <table border="1" cellpadding="4">
    <tr>
      <th>Tipo / Ubicación</th>
      ${ubicaciones.map((u) => `<th>${u}</th>`).join("")}
      <th>Total</th>
    </tr>
`;
...
tipos.forEach((tipo) => {
  ...
  ubicaciones.forEach((ubi) => {
    const count = datosInmobiliarios.filter(
      (d) => d.TipoPropiedad === tipo && d.Ubicacion === ubi
    ).length;
    totalTipo += count;
...
// Fila de totales por ubicación
tablaContHTML += `<tr><td><b>Total</b></td>`;
let totalGeneral = 0;
ubicaciones.forEach((ubi) => {
  const count = datosInmobiliarios.filter((d) => d.Ubicacion === ubi).length;
  totalGeneral += count;
  tablaContHTML += `<td>${count}</td>`;
});
tablaContHTML += `<td>${totalGeneral}</td></tr>`;
tablaContHTML += "</table>";

document.getElementById("tablaContingencia").innerHTML = tablaContHTML;

// 6. Conclusiones (ejemplo)
resultadosPropiedadesDiv.innerHTML += `
  <h3>Conclusiones:</h3>
  <ol>
    <li>La mayoría de las propiedades tienen precios entre ${labelsPrecios[0]} y ${labelsPrecios[1]}.</li>
    <li>El tipo de propiedad más frecuente es <b>${moda(datosInmobiliarios.map(d => d.TipoPropiedad))}</b>.</li>
...
function moda(arr) {
  ...
  return modas.length === 1 ? modas[0] : modas.join(", ");
}
